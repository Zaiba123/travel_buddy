	...
	onSelect = async data => {
		...
		if (response.data.results && response.data.results.length > 0) {
			...
			for (let i = 0; i < response.data.results.length; i++) {
				const event = response.data.results[i];
				const dupEvent = events.find(item => event.title === item.title);
				if (!dupEvent) {
					events.push(event);
      ...
      while (response.data.next) {
        httpOptions.params.next = response.data.next;
        response = await axios.get(
          "http://localhost:9000/get-events",
          httpOptions
        );
        httpOptions.params.next = null;
				...
				this.state.weather ? (
					<div className="container">
                        ...
                        <Card style={{ width: '370px' }} className='mb-4'>
                          <Card.Body>
                            <Card.Title as={Link} to={`/event/${event.id}`} className='h5'>{event.title}</Card.Title>
                            <Card.Subtitle className="my-2 text-muted">{event.entities.length > 0 && event.entities[0].formatted_address}</Card.Subtitle>
                          </Card.Body>
                        </Card>
                      </div>
										))}
						...
						<div className="weather">
							<h3>Weather</h3>
							<p>
								Sky: {this.state.weather.weather_descriptions[0]}{" "}
								<img
							...
							<p>Temperature: {this.state.weather.temperature} C</p>
							<p>Feels Like: {this.state.weather.feelslike} C</p>
